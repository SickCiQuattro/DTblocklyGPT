...
# Tag che la regex non sa leggere, mappati a mano. Gemma 3n dichiara un
# "effective parameter count": e4b gira con la memoria di un modello da 4
# miliardi pur avendone circa otto, perche' l'architettura MatFormer ne tiene
# fuori una parte a ogni passo. Sull'asse della figura conta quanto costa
# tenerlo in memoria, quindi vale il numero effettivo. Senza questa riga il
# modello locale piu' accurato dell'intera matrice sparisce dalla figura sulla
# scala, che e' esattamente il punto in cui serve.
_SIZE_OVERRIDE = {
    "ollama:gemma4:e4b-it-qat": 4.0,
    "ollama-nothink:gemma4:12b-it-qat": 12.0,
}
...
# Modelli tenuti fuori dai grafici che elencano un modello per riga, cioe'
# pass_rate_per_model e category_heatmap. Restano misurati, restano nel
# report a console e nelle tabelle dell'appendice: escono solo dalle due
# figure dove costano una riga a testa e non portano un confronto.
# granite4:350m sta al 2,0% e qwen2.5:3b al 22,4%, mentre gli altri undici
# locali stanno fra il 59 e il 94: due righe quasi vuote comprimono in meta'
# immagine la fascia dove il confronto avviene davvero.
# NON si applicano a size_vs_accuracy, dove il punto della figura e' proprio
# che cosa succede alle dimensioni minime: li' togliere qwen2.5:3b
# cancellerebbe l'estremo basso della serie qwen2.5 3B-7B-14B.
CHART_EXCLUDE = ("ollama:granite4:350m", "ollama:qwen2.5:3b")


def _chart_specs(all_results, want_local):
    return [s for s in all_results
            if _is_local(s) == want_local and s not in CHART_EXCLUDE]
...
PALETTE = {
    # "red" e' il polo caldo delle polarita' (prima/dopo, EN/IT, thinking/
    # nothink). Terracotta e non vermiglio: entrambi superano le verifiche
    # accanto al blu, ma #C2593C sta nel registro del Neutro Caldo del brand
    # (#F7F5EB) invece di scavalcarlo. Misure del validatore del design system,
    # accanto a "blue": delta-E 25,3 a visione normale e 17,8 in protanopia,
    # su una soglia di 15. Il vermiglio #e34948 che sostituisce dava 29,8 e
    # 16,4: piu' distante a vista piena, meno a visione ridotta.
    "blue": "#445392", "orange": "#eb6834", "red": "#C2593C",
}
INK = {
...
def _apply_thesis_style():
    ...
    plt.rcParams.update({
        ...
        # Arial e' il font della tesi, prescritto dal manuale di identita'
        # visiva UNIBS per i documenti (Brand Guidelines 2026, pag. 37 e 44).
        # Senza questa riga matplotlib ripiega su DejaVu Sans, e i grafici
        # incorporati con \includegraphics sono l'unica parte del capitolo
        # composta in un carattere diverso dal resto. Il ripiego resta in
        # coda alla lista per le macchine senza Arial.
        "font.family": "sans-serif",
        "font.sans-serif": ["Arial", "Liberation Sans", "Helvetica", "DejaVu Sans"],
        "pdf.fonttype": 42,  # TrueType incorporato, non Type 3: il testo
                             # resta selezionabile e cercabile nel PDF finale
        "font.size": 10,
...
def report_determinism(all_results):
    ...
    The runs are recovered by occurrence index within each case name, NOT by
    slicing the merged rows into consecutive blocks of GOLDEN_SET_SIZE. Most
    files are stored run-major and the two agree, but a file whose cases were
    re-measured and appended is not: ollama:deepseek-r1:14b carries a
    _patch_meta block for five cases re-run after the prose-bucket fix, and
    index slicing cut its 245 rows into "runs" holding 44, 44, 44, 44 and 29
    distinct cases. That produced an sd of 4.56 for a model whose every case
    has the same outcome in all five runs. Grouping by name is order-free and
    gives the same answer on a clean file.
    ...
    for spec in sorted(all_results, key=lambda s: len(all_results[s])):
        ...
        # Run i is the i-th occurrence of every case, so a case measured fewer
        # than k times simply does not contribute to the later runs.
        blocks = [[v[i] for v in by_case.values() if len(v) > i] for i in range(k)]
        rates = [100.0 * sum(b) / len(b) if b else 0.0 for b in blocks]
...
def _delivery(results):
    """Split failures into wrong answers and answers that never arrived.

    A call that ends in a transport failure — HTTP 503 from a congested
    endpoint, a timeout, a provider that bills tokens and returns nothing —
    tells you about the service, not about the model. Counting it as a failed
    case reports the provider's availability as the model's accuracy, and on
    this campaign that is not a marginal correction: two cloud rows got every
    delivered answer right and still scored in the fifties and sixties.

    Returned: delivered calls, undelivered calls, and the pass rate over the
    delivered ones only.
    """
    undelivered = [r for r in results if r.get("error")]
    delivered = [r for r in results if not r.get("error")]
    rate = (100.0 * sum(1 for r in delivered if r.get("pass")) / len(delivered)
            if delivered else 0.0)
    return delivered, undelivered, rate


def report_delivery(all_results, out_dir=None):
    print("\n" + "=" * 70)
    print("RISPOSTE CONSEGNATE CONTRO RISPOSTE MANCATE")
    print("=" * 70)
    print(f"  {'spec':<38}{'grezzo':>8}{'consegnate':>12}{'delta':>8}{'503':>6}{'timeout':>9}{'altri':>7}")
    any_gap = False
    for spec, results in all_results.items():
        delivered, undelivered, rate = _delivery(results)
        if not undelivered:
            continue
        any_gap = True
        raw = _pass_rate(results)
        e503 = sum(1 for r in undelivered if "503" in str(r.get("error", "")))
        eto = sum(1 for r in undelivered if "timed out" in str(r.get("error", "")).lower())
        print(f"  {spec:<38}{raw:>7.1f}%{rate:>11.1f}%{rate - raw:>+7.1f}"
              f"{e503:>6}{eto:>9}{len(undelivered) - e503 - eto:>7}")
    if not any_gap:
        print("  nessun modello ha risposte mancate")
        return
    print("\n  Una riga con molte risposte mancate non e' una misura del modello.")
    print("  Il tasso sulle consegnate e' quello da citare, con n dichiarato.")
    _delivery_chart(all_results, out_dir)


def _delivery_chart(all_results, out_dir):
    """delivery — lo stesso insieme di chiamate contato in due modi.

    Non e' un prima/dopo: nessuna esecuzione e' stata rifatta. Il tasso grezzo
    conta come fallito ogni caso la cui risposta non e' mai arrivata, quello
    sulle consegnate lo esclude. La differenza fra i due dice quanto della
    riga misura il fornitore invece del modello.

    La figura porta n accanto a ogni barra perche' i due 100% qui dentro
    stanno su esecuzioni parziali: leggerli senza numerosita' accanto e'
    esattamente la lettura sbagliata che la figura esiste per impedire.
    """
    if plt is None or not out_dir:
        return
    rows = []
    for spec, results in all_results.items():
        delivered, undelivered, rate = _delivery(results)
        if undelivered:
            rows.append((spec, _pass_rate(results), rate, len(delivered), len(results)))
    rows.sort(key=lambda r: r[1] - r[2])  # scarto maggiore in cima

    fig, ax = plt.subplots(figsize=(8.6, max(3.2, len(rows) * 0.62 + 1.4)))
    y = list(range(len(rows)))
    h = 0.34
    ax.barh([v + h / 2 for v in y], [r[1] for r in rows], height=h,
            color=INK["muted"], zorder=3, label="tutte le chiamate (mancate contate come fallite)")
    ax.barh([v - h / 2 for v in y], [r[2] for r in rows], height=h,
            color=PALETTE["blue"], zorder=3, label="sole risposte consegnate")
    for yi, (_spec, raw, rate, nd, ntot) in zip(y, rows):
        ax.text(raw + 1.5, yi + h / 2, f"{raw:.1f}  (n={ntot})", va="center",
                fontsize=8, color=INK["secondary"])
        ax.text(rate + 1.5, yi - h / 2, f"{rate:.1f}  (n={nd})", va="center", fontsize=8)
    ax.set_yticks(y)
    ...
    plt.tight_layout()
    _save(fig, out_dir, "delivery",
          title="Risposte consegnate contro risposte mancate",
          note="Stessa esecuzione contata in due modi, non due esecuzioni. "
               "n = chiamate su cui il tasso e' calcolato.")
...
def report_size_vs_accuracy(all_results, out_dir):
    ...
    for spec, results in all_results.items():
        if not _is_local(spec):
            continue
        if spec in _SIZE_OVERRIDE:
            size_b = _SIZE_OVERRIDE[spec]
        else:
            m = _SIZE_RE.search(spec)
            if not m:
                print(f"  (fuori figura, dimensione non leggibile dal tag: {spec})")
                continue
    ...
    label_offsets = {
        "granite4:350m": (10, 0, "left", "center"),
        "qwen2.5:3b": (10, -2, "left", "center"),
        "gemma4:e4b-it-qat": (-10, 0, "right", "center"),
        "qwen2.5:7b": (-10, 4, "right", "bottom"),
        "qwen2.5-coder:7b": (0, 12, "center", "bottom"),
        "granite4.1:8b": (0, -16, "center", "top"),
        "llama3.1:8b": (12, 0, "left", "center"),
        "qwen3.5:9b": (10, -4, "left", "center"),
        "qwen3.5:9b (nothink)": (0, 12, "center", "bottom"),
        "ministral-3:8b": (-10, 0, "right", "center"),
        "gemma4:12b-it-qat (nothink)": (0, -16, "center", "top"),
        "deepseek-r1:14b": (-8, 12, "right", "bottom"),
        "qwen2.5:14b": (-10, -4, "right", "center"),
    }
...
def report_pass_rate_bar(all_results, out_dir, baseline_spec="gemini:gemini-3.5-flash-lite"):
    """pass_rate_per_model — accuratezza complessiva, due pannelli affiancati.

    Una figura sola con (a) remoti e (b) locali sullo stesso asse x. Erano due
    immagini separate, e la separazione nascondeva proprio l'affermazione che
    il capitolo fa: le due fasce vanno lette una contro l'altra, e girare
    pagina per farlo chiede al lettore di ricordare undici numeri.

    Le barre hanno una tinta sola, perche' l'identita' la porta gia'
    l'etichetta sull'asse y; la baseline di produzione e' segnata con
    etichetta in grassetto e bordo scuro invece che con un secondo colore,
    cosi' il rosso resta riservato a una polarita' vera altrove nel capitolo.
    La famiglia locale non ha una baseline di produzione e non ha nulla di
    evidenziato.
    """
    print("\n" + "=" * 70)
    print("PASS RATE PER MODEL (bar chart, due pannelli)")
    print("=" * 70)
    full = EXPECTED_RUNS * GOLDEN_SET_SIZE
    panels = []
    for family, want_local in (("remoti", False), ("locali", True)):
        rows = [(spec, _pass_rate(all_results[spec]), len(all_results[spec]))
                for spec in _chart_specs(all_results, want_local)]
        rows.sort(key=lambda r: r[1])
        print(f"  -- {family} --")
        for spec, rate, n in rows:
            # La copertura si segnala in ENTRAMBE le direzioni. Un modello
            # misurato su piu' chiamate del protocollo e' sospetto quanto uno
            # misurato su meno: in questi dati l'unico caso simile sono due
            # finestre di contesto diverse fuse sotto un nome solo, che un
            # controllo "< esecuzione piena" lascerebbe passare.
            flag = "" if n == full else f" (copertura {n}/{full})"
            print(f"    {spec}: {rate:.1f}%{flag} (n={n})")
        panels.append((family, rows))
    for spec in CHART_EXCLUDE:
        if spec in all_results:
            print(f"  (fuori figura: {spec} al {_pass_rate(all_results[spec]):.1f}%)")

    if plt is None:
        return
    height = max(4.0, max(len(rows) for _, rows in panels) * 0.40 + 1.0)
    fig, axes = plt.subplots(1, 2, figsize=(11, height))
    for ax, letter, (family, rows) in zip(axes, "ab", panels):
        labels = [_short(spec) + ("" if n == full else " *") for spec, _, n in rows]
        values = [r[1] for r in rows]
        edge = [INK["primary"] if spec == baseline_spec else "none" for spec, _, _ in rows]
        lw = [1.6 if spec == baseline_spec else 0 for spec, _, _ in rows]
        y = list(range(len(rows)))
        ax.barh(y, values, color=PALETTE["blue"], edgecolor=edge, linewidth=lw,
                height=0.6, zorder=3)
        ax.set_yticks(y)
        ...
        ax.set_xlim(0, 112)
        ax.set_xlabel("pass rate (%)")
        # L'identificatore di pannello resta nell'immagine: una didascalia non
        # puo' indicare il pannello di sinistra.
        ax.set_title(f"({letter}) {family}", loc="left", fontsize=10)
        _style_axes(ax, grid_axis="x")
    notes = []
    if any(spec == baseline_spec for _, rows in panels for spec, _, _ in rows):
        notes.append(f"Contorno in grassetto: baseline di produzione ({_short(baseline_spec)}).")
    if any(n != full for _, rows in panels for _, _, n in rows):
        notes.append(f"* copertura diversa dalle {full} chiamate del protocollo.")
    plt.tight_layout()
    _save(fig, out_dir, "pass_rate_per_model",
          title="Accuratezza complessiva per modello",
          note="  ".join(notes) if notes else None)


def report_category_heatmap(all_results, out_dir):
    """category_heatmap — pass rate per categoria x modello, una matrice sola.

    Erano due immagini, una per famiglia, sulla stessa scala cromatica perche'
    si potessero confrontare. Ora sono una matrice sola con una riga vuota fra
    le famiglie, e il motivo e' la colonna hallucination_guard: e' li' che le
    due famiglie si separano, e una separazione si legge se le due meta' sono
    adiacenti, non se stanno su due pagine con la stessa legenda.

    Entra in una pagina perche' i due modelli minimi stanno fuori
    (CHART_EXCLUDE): con loro le righe sarebbero ventiquattro, e due di quelle
    righe sono quasi tutte zeri.
    """
    print("\n" + "=" * 70)
    ...
    groups = []
    for family, want_local in (("remoti", False), ("locali", True)):
        specs = _chart_specs(all_results, want_local)
        specs.sort(key=lambda sp: _pass_rate(all_results[sp]))
        if specs:
            groups.append((family, specs))
    if not groups:
        print("  (nessun modello fra i file passati)")
        return

    seen = set()
    for _, specs in groups:
        for sp in specs:
            for r in all_results[sp]:
    ...
    labels, matrix = [], []
    for gi, (family, specs) in enumerate(groups):
        if gi:  # riga vuota di separazione fra le due famiglie
            labels.append("")
            matrix.append([float("nan")] * len(categories))
        print(f"  -- {family} --")
        for sp in specs:
            row = []
            for c in categories:
                rs = [r for r in all_results[sp] if (r.get("category") or "uncategorized") == c]
                row.append(_pass_rate(rs) if rs else float("nan"))
            labels.append(_short(sp))
            matrix.append(row)
            print(f"    {sp}: " + " | ".join(f"{c}={v:.0f}%" for c, v in zip(categories, row)))

    if plt is None:
        return
    arr = np.array(matrix)
    fig, ax = plt.subplots(figsize=(1.15 * len(categories) + 3.4, 0.36 * len(labels) + 2.0))
    ...
    for i in range(len(labels)):
        for j in range(len(categories)):
            v = arr[i, j]
            if v != v:  # NaN: categoria assente, o riga di separazione
                continue
            ax.text(j, i, f"{v:.0f}", ha="center", va="center", fontsize=8,
                    color="white" if v > 55 else INK["primary"])
    # Il nome della famiglia sul margine sinistro, dove la riga vuota la stacca.
    row = 0
    for gi, (family, specs) in enumerate(groups):
        if gi:
            row += 1
        ax.text(-0.5, row - 0.75, f"({'ab'[gi]}) {family}", ha="left", va="bottom",
    ...
    _save(fig, out_dir, "category_heatmap",
          title="Accuratezza per categoria e modello",
          note="n = casi per categoria in una singola esecuzione. Fuori figura i due "
               "modelli locali minimi, riportati in appendice.")
...
def report_thinking_toggle(all_results, out_dir, pair=("ollama:qwen3.5:9b", "ollama-nothink:qwen3.5:9b")):
    """La coppia ceteris paribus sul ragionamento nascosto: stessi pesi, cambia
    solo il flag think:true/false della richiesta. Emette thinking_toggle_by_category."""
    print("\n" + "=" * 70)
    ...
    # I due numeri complessivi restano a console e nel testo del capitolo. La
    # figura che li disegnava (thinking_toggle_qwen35_9b) e' stata tolta: due
    # barre per due numeri che una frase porta meglio, e la scomposizione per
    # categoria qui sotto li contiene entrambi dicendo anche da dove vengono.
...
def main():
    ...
    report_delivery(all_results, args.out_dir)
