...
const getMacroIdFromBlockData = (rawData: unknown) => {
...
const parseMacroSteps = (rawCode: string) => {
...
const getMenuIconInfo = (text: string) => {
  ...
  if (containsAny(['expand macro', 'esplodi macro'])) {
...
export const CustomDragDrop = ({
  ...
  const explodeMacro = useCallback(
    (block: Blockly.BlockSvg, workspace: Blockly.WorkspaceSvg) => {
      ...
      // Understands whether it is Abstract JSON or Blockly JSON
      if (
      ...
      // SAVING PHYSICAL STATE (Connections and Coordinates)
      ...
      const xyCoordinates = block.getRelativeToSurfaceXY() // save position to restore for flyout blocks

      const currentEventGroup = Blockly.Events.getGroup()
      if (!currentEventGroup) {
        Blockly.Events.setGroup(true) // group all events generated by this operation under a single group for better undo/redo behavior
      }

      try {
        // Unlink the underlying block before destroying the macro,
        // otherwise Blockly will destroy the entire underlying tree
        if (block.nextConnection && block.nextConnection.targetConnection) {
          block.nextConnection.disconnect()
        }

        // Destroys the macro (false = do not automatically stitch blocks together)
        block.dispose(false)

        // Inject the exploded contents of the macro
        ...
        if (!newFirstBlock)
          throw new Error('Blockly non è riuscito a renderizzare lo stato.')

        // RECONNECTION
        if (prevConnection && newFirstBlock.previousConnection) {
          // Attach the top to the previous block
          prevConnection.connect(newFirstBlock.previousConnection)
        } else {
          // If it was a flying block, return it to the exact coordinates it was located at.
          newFirstBlock.moveTo(xyCoordinates)
        }

        // Find the last block of the chain just exploded
        ...
        if (nextConnection && newLastBlock?.nextConnection) {
          // Reattach the code that was under the last exploded block
        ...
        if (sourceBlock.type === 'macro_task_block') {
          const explodeOption = {
            ...
            enabled:
              ...
              !!getMacroIdFromBlockData(sourceBlock.data),
            scope: {
              block: sourceBlock,
            ...
            callback: (scope: Blockly.ContextMenuRegistry.Scope) => {
              const blockFromScope = scope?.block as
                | Blockly.BlockSvg
                | undefined
              ...
              explodeMacro(blockFromScope, workspaceFromScope)
            },
          }
          ...
          const deleteOptionIndex = menuOptions.findIndex((option) => {
            ...
            return (
              optionText.includes('delete') ||
              optionText.includes('elimina') ||
              optionText.includes('cancella')
            )
          })
